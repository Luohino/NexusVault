...
function ToolbarView:new()
  ...
  self.toolbar_commands = {
    {symbol = "g", command = "nv:init"},
    {account_icon = true, command = "nv:settings"},
...
function ToolbarView:get_icon_width()
  ...
  for i,v in ipairs(self.toolbar_commands) do 
    ...
    elseif v.icon or v.account_icon then
      max_width = math.max(max_width, 24)  -- Default icon width
    end
  end
  return max_width > 0 and max_width or 24
end
...
local nexus_logo_map = nil
pcall(function() nexus_logo_map = require "plugins.nexusvault_map" end)

local function draw_account_icon(x, y, w, h, color)
  local unit = math.max(1, math.floor(h / 16))
  local cx = x + w / 2
  local top = y + math.floor(h * 0.2)

  renderer.draw_rect(cx - 3 * unit, top, 6 * unit, 2 * unit, color)
  renderer.draw_rect(cx - 4 * unit, top + 2 * unit, 8 * unit, 4 * unit, color)
  renderer.draw_rect(cx - 3 * unit, top + 6 * unit, 6 * unit, 2 * unit, color)
  renderer.draw_rect(cx - 7 * unit, top + 10 * unit, 14 * unit, 2 * unit, color)
  renderer.draw_rect(cx - 9 * unit, top + 12 * unit, 18 * unit, 3 * unit, color)
end
...
function ToolbarView:draw()
  ...
  for item, x, y, w, h in self:each_item() do
    ...
    if item.icon and item.icon:find("nexusvault.png") and nexus_logo_map then
      -- Draw custom NexusVault Logo from Pixel Map (Balanced Scale)
      local scale = (h * 1.3) / 32
      local ox, oy = x + (w - 32 * scale) / 2, y + (h - 32 * scale) / 2
      for _, p in ipairs(nexus_logo_map) do
        renderer.draw_rect(ox + p[1] * scale, oy + p[2] * scale, scale + 0.3, scale + 0.3, {p[3], p[4], p[5], p[6]})
      end
    elseif item.account_icon then
      draw_account_icon(x, y, w, h, color)
    else
      common.draw_text(item.font or self.toolbar_font, color, item.symbol or "!", nil, x, y, 0, h)
    end
  end
end
