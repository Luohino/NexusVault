...
local function should_show_hidden_file(name)
  return name == ".nvignore"
end


local NVIGNORE_PROTECTED_PATTERNS = { ".nv", ".git" }
local NVIGNORE_DEFAULT_PATTERNS = { "node_modules", "dist" }
...
local function nvignore_path_for(root)
  return root .. (PATHSEP or "/") .. ".nvignore"
end

local function find_open_nvignore_doc(root)
  local target = normalize_for_compare(nvignore_path_for(root))
  for _, doc in ipairs(core.docs or {}) do
    if doc.abs_filename and normalize_for_compare(doc.abs_filename) == target then
      return doc
    end
  end
  return nil
...
local function parse_nvignore_content(content)
  content = content or ""
  local patterns = {}
...
local function get_nvignore_source(root)
  local doc = find_open_nvignore_doc(root)
  if doc then
    return table.concat(doc.lines or {}, ""), "doc:" .. tostring(doc:get_change_id())
  end
  local path = nvignore_path_for(root)
  local stat = system.get_file_info(path)
  local modified = stat and stat.modified or "missing"
  return read_text_file(path) or "", "file:" .. tostring(modified)
end

local function matches_nvignore_pattern(rel_path, pattern)
...
local function is_nv_protected_ignored(rel_path)
  for _, pattern in ipairs(NVIGNORE_PROTECTED_PATTERNS) do
    if matches_nvignore_pattern(rel_path, pattern) then return true end
  end
  return false
end

local function is_nv_default_ignored(rel_path)
  for _, pattern in ipairs(NVIGNORE_DEFAULT_PATTERNS) do
    if matches_nvignore_pattern(rel_path, pattern) then return true end
  end
  return false
end

local function is_nvignore_ignored(rel_path, patterns)
  if rel_path == "" then return false end
  rel_path = rel_path:gsub("\\", "/")
  if is_nv_protected_ignored(rel_path) then return true end
  local ignored = is_nv_default_ignored(rel_path)
  for _, raw in ipairs(patterns) do
    local negated = raw:sub(1, 1) == "!"
    local pattern = negated and raw:sub(2) or raw
    if matches_nvignore_pattern(rel_path, pattern) then
      ignored = not negated
    end
  end
  return ignored
...
local function get_nvignore_patterns(view, project)
  if not view.nvignore_cache then view.nvignore_cache = {} end
  local content, signature = get_nvignore_source(project.path)
  local cached = view.nvignore_cache[project.path]
  if not cached or cached.signature ~= signature then
    cached = { signature = signature, patterns = parse_nvignore_content(content) }
    view.nvignore_cache[project.path] = cached
  end
  return cached.patterns
end

local function refresh_nvignore_cache(view)
  if not view.nvignore_signatures then view.nvignore_signatures = {} end
  for _, project in ipairs(core.projects or {}) do
    local _, signature = get_nvignore_source(project.path)
    if view.nvignore_signatures[project.path] ~= signature then
      view.nvignore_signatures[project.path] = signature
      view.nvignore_cache = {}
      view.cache = {}
      break
    end
...
local function is_nexusvault_ignored(view, project, path)
  if not project or not project.path or not path then return false end
  local root = project.path:gsub("\\", "/")
  local rel_path = path:gsub("\\", "/")
  if rel_path == root then return false end
  if rel_path:find(root .. "/", 1, true) == 1 then
    rel_path = rel_path:sub(#root + 2)
  end
  return is_nvignore_ignored(rel_path, get_nvignore_patterns(view, project))
end
...
function TreeView:get_cached(project, path)
  ...
  if not t then
    ...
    t = {
      ...
      type = info.type,
      project = project,
      ignored = self.show_ignored and (project:is_ignored(info, path) or is_nexusvault_ignored(self, project, path))
    }
  ...
  if t.expanded and t.type == "dir" and not t.files then
    ...
    for i, file in ipairs(system.list_dir(path)) do
      ...
      if f and f.type then
        f.name = file
        f.abs_filename = l
        f.ignored = self.show_ignored and (project:is_ignored(f, l) or is_nexusvault_ignored(self, project, l))
...
function TreeView:get_items(project, path, x, y, w, h)
  ...
  if dir and dir.files and dir.expanded then
    for i, file in ipairs(dir.files) do
      if self.show_hidden or should_show_hidden_file(file.name) or not file.name:find("^%.") then
...
function TreeView:on_context_menu()
  return { items = {
    { text = "Reveal in Explorer", command = "treeview:open-in-system" },
    { text = "Copy Path", command = "treeview:copy-path" },
    ContextMenu.DIVIDER,
    ...
    { text = "Remove Project Directory", command = "treeview:remove-project-directory" },
  } }, self
end
  ...
  ["treeview:open-in-system"] = function()
    ...
    if item then
      if PLATFORM == "Windows" then
        system.exec(string.format("explorer /select,%q", item.abs_filename))
      else
        system.exec(string.format("open %q", item.abs_filename))
      end
    end
  end,
  ["treeview:copy-path"] = function()
    local item = treeitem()
    if item then
      system.set_clipboard(item.abs_filename)
      core.log("Copied path to clipboard: %s", item.abs_filename)
    end
  end,
})
