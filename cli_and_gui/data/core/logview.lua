...
function LogView:on_mouse_pressed(button, px, py, clicks)
  ...
  if button == "right" then
    local items = self:on_context_menu(px, py)
    core.root_view.context_menu:show(px, py, items.items)
    return true
  end
...
command.add(LogView, {
  ["log:copy-all"] = function()
    ...
    for _, item in ipairs(core.log_items) do
      table.insert(t, core.get_log(item))
    end
    system.set_clipboard(table.concat(t, "\n"))
    core.log("Copied all logs to clipboard.")
  end,
  ["log:clear"] = function()
    core.log_items = {}
    core.log("Logs cleared.")
  end
})

function LogView:on_context_menu(px, py)
  return {
    items = {
      { text = "Copy All", command = "log:copy-all" },
      { text = "Clear Logs", command = "log:clear" },
    }
  }, self
end
