...
local function draw_suggestions_box(self)
  ...
  if #self.suggestions > 0 then
    renderer.draw_rect(rx, ry, rw, rh, style.syntax["normal"]) -- Pure White
    renderer.draw_rect(rx, ry - dh, rw, dh, style.divider)
    local y = self.position.y - self.selection_offset - dh
    renderer.draw_rect(rx, y, rw, lh, style.accent) -- Selection highlight (Red)
  end
  ...
  for i=first, last do
    ...
    local color = (i == self.suggestion_idx) and style.syntax["normal"] or style.background -- White if selected, Black if not
    local y = self.position.y - (i - first + 1) * lh - dh
    common.draw_text(self:get_font(), color, item.text, nil, x, y, 0, lh)

    if item.info then
      local info_color = (i == self.suggestion_idx) and style.syntax["normal"] or style.dim
      local w = self.size.x - x - style.padding.x
      common.draw_text(self:get_font(), info_color, item.info, "right", x, y, w, lh)
    end
  end
  core.pop_clip_rect()
end
...
function CommandView:draw_background(color)
  CommandView.super.draw_background(self, style.accent)
end
